...
private:
	...
	T *m_dptr[2];
	GLuint m_vbo[2];
	struct cudaGraphicsResource *m_cuda_vbo_resource[2];
...
template <class T>
void
GpuArray<T>::alloc(size_t size, bool vbo/* =false */, bool doubleBuffer/* =false */,bool useElementArray/* =false */)
{
	m_size=size;
...
GpuArray<T>::allocVbo(bool useElementArray)
{
	...
	cutilSafeCall(cudaGraphicsGLRegisterBuffer(&m_cuda_vbo_resource[0],m_vbo[0],
							cudaGraphicsMapFlagsWriteDiscard));

	if (m_doubleBuffer)
	{
		m_vbo[1]=createVbo(m_size*sizeof(T),useElementArray);
		cutilSafeCall(cudaGraphicsGLRegisterBuffer(&m_cuda_vbo_resource[1],m_vbo[1],
							cudaGraphicsMapFlagsWriteDiscard));
	}
}
...
GpuArray<T>::freeVbo()
{
	if (m_vbo[0])
	{
		cutilSafeCall(cudaGraphicsUnregisterResource(m_cuda_vbo_resource[0]));
	...
	if (m_vbo[1])
	{
		cutilSafeCall(cudaGraphicsUnregisterResource(m_cuda_vbo_resource[1]));
...
GpuArray<T>::map()
{
	if (m_vbo[0])
	{
		cutilSafeCall(cudaGraphicsMapResources(1,&m_cuda_vbo_resource[0],0));
		size_t num_bytes;
		cutilSafeCall(cudaGraphicsResourceGetMappedPointer((void **)&m_dptr[0], &num_bytes,
									m_cuda_vbo_resource[0]));
	}
	if (m_doubleBuffer && m_vbo[1])
	{
		cutilSafeCall(cudaGraphicsMapResources(1,&m_cuda_vbo_resource[1],0));
		size_t num_bytes;
		cutilSafeCall(cudaGraphicsResourceGetMappedPointer((void **)&m_dptr[1], &num_bytes,
...
GpuArray<T>::unmap()
{
	if (m_vbo[0])
	{
		cutilSafeCall(cudaGraphicsUnmapResources(1,&m_cuda_vbo_resource[0],0));
		m_dptr[0]=0;
	}
	if (m_doubleBuffer && m_vbo[1])
	{
		cutilSafeCall(cudaGraphicsUnmapResources(1,&m_cuda_vbo_resource[1],0));
		m_dptr[1]=0;
	}
}
...
template <class T>
void
GpuArray<T>::copy(Direction dir, uint start/* =0 */,uint count/* =0 */)
{
	if (count==0)
	{
		count = (uint) m_size;
...
template <class T>
void
GpuArray<T>::memset(T value, uint start/* =0 */,uint count/* =0 */)
{
