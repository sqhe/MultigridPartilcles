...
void
GLSLProgram::setUniform1f(const GLchar *name, float value)
{
...
void
GLSLProgram::setUniform2f(const GLchar *name, float x,float y)
{
	GLint loc = glGetUniformLocation(mProg,name);
	if (loc >= 0)
	{
...
void
GLSLProgram::setUniformfv(const GLchar *name,GLfloat *v, int elementSize, int count/* =1 */)
{
	GLint loc = glGetUniformLocation(mProg,name);
	if (loc == -1)
	{
